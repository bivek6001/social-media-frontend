...
<div className="person user-info relative bg-[#202C33] w-[100%] h-[70px] flex justify-start items-center gap-3 px-4">
    <div className="img relative">
        <img src={selectedUser?.profilePicture} className='w-[50px] h-[50px] rounded-full object-cover' alt="user-chat-dp" />
      
       
    </div>
   
    <span className='flex p-2 items-center justify-center text-white font-medium gap-1'> <div className={`h-[10px] w-[10px]  ${onlineUsers?.includes(selectedUser?._id) ?  "bg-green-500" : "bg-red-600"}  rounded-full`}></div> {onlineUsers?.includes(selectedUser?._id) ?  "online" : "offline"}  </span> 
...
<div className="input absolute bottom-0 bg-[#202C33] w-[100%] h-[12%] flex justify-center items-center">
    <form method="post" className='w-[100%] p-2 flex justify-center items-center' onSubmit={sendMessage}>
        <input type="text" placeholder='Type a message' className='bg-[#2A3942] text-white outline-none w-[90%] p-2 rounded-md' value={message} onChange={(e)=>{
